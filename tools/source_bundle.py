...
INVENTORY_DIGEST = 'a6ec753e2666666783a4f1d9d0bde9adfa08c68428a700b112824ac484f0f78e'
HERE = Path(__file__).resolve().parent
TOOL_FILES = ('source_bundle.py', 'source_inventory.json', 'run_fft1b.py',
              'verify_fft1b.py', 'compare_repeat.py', 'repeat_policy.json',
              'REPEATABILITY_POLICY.md', 'README_ZH.md')
...
def export_bundle(bundle, target):
    ...
    identity = {
        'commit': doc['commit'], 'tree': doc['tree'],
        'export': 'selected original blobs verified against pinned reviewed inventory; not full tree',
        'inventory_sha256': INVENTORY_DIGEST,
        'archive_sha256': hashlib.sha256(bundle.read_bytes()).hexdigest(),
        'build_marker': '.human-cos-checked-out-commit',
    }
    (target.parent / 'source-identity.json').write_text(json.dumps(identity, indent=2) + '\n')
...
def build(root, output):
    payloads = {'runtime-source.zip': source_bytes(root),
                'run_trial.py': LAUNCHER.encode(), 'START_HERE.md': NOTICE.encode()}
    ...
    payloads['PACKAGE_SHA256.json'] = (json.dumps(manifest, indent=2, sort_keys=True) + '\n').encode()
...
if __name__ == '__main__':
    ...
    print(json.dumps({'archive': str(args.output), 'sha256': build(args.source_root, args.output),
